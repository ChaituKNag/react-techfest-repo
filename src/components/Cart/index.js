...
class Cart extends Component {
  ...
  proceedCheckout() {
    ...
    var checkoutItems = cartData.map(eachProduct => ({ quantity: eachProduct.quantity, productId: eachProduct.productId }));
    var checkoutObj = {
      userId: 1,
      statusId: 1,
      items: checkoutItems
            ...
            <div className="cart-page">
                <Container>
                    ...
                    { cartData && cartData.length > 0 ? (<div className="total-price-section">
                        ...
                        <p className="grand-total"><span>Grand Total:</span><Price currency="$" price={this.props.subTotal + 800}/></p>
                        <Cta className="btn btn-outline-warning" ctaText="Continue Shopping" ctaType="link" ctaPath="/"></Cta>
                        <Cta ctaColor="warning" ctaText="Proceed To Checkout" ctaType="button" onClickHandler={this.proceedCheckout.bind(this)}></Cta>
        </div>) : <p className="cart-empty-message">Your bag is empty !!!</p> }
                  {this.props.orderStatus !== '' && 
                        <div className="status-message">
                          <span>{this.props.orderStatus ? 'Your order has been placed successfully.' : 'Your order cannot be placed.' }</span>
                        </div> 
                      }
                </Container>
            </div>
